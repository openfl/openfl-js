...
// TODO: Re-use code from SWF project
import haxe.Json;
...
// import swf.exporters.swflite.SWFLibrary;
									...
									if (object.name != null && !objectReferences.exists(object.name)) {
										if (swfLite.symbols.exists(object.symbol)) {
											...
											if (className == null) {
												...
												} else if (Std.is(childSymbol, DynamicTextSymbol)
													|| Std.is(childSymbol, StaticTextSymbol)) {
													className = "openfl.text.TextField";
...
<library>
	...
	private static function processLibraries(project:HXProject):HXProject {
		...
		if (embeddedSWFLite) {
			...
			// output.haxeflags.push ("--macro include('swf.exporters.swflite.SWFLiteLibrary')");
